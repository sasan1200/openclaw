  ...
  const lines = [
    ...
    hasGateway && !isMinimal
      ? [
          ...
          "Prefer agents.create, agents.update, and agents.delete for agent roster changes instead of patching agents.list directly.",
          "Actions: config.schema.lookup, config.get, config.apply (validate + write full config, then restart), config.patch (partial update, merges with existing), agents.create, agents.update, agents.delete, update.run (update deps or git, then restart).",
